...
<details>
  ...
  <p><blockquote>The web server connected to Sinatra is running in your development environment, so the first part of the URL is something like <code>http://aaa.bbb.ccc.ddd</code>. The Ruby code <code>get '/new' do...</code> in <code>app.rb</code> renders the New Game page, so the full URL is in the form <code>http://aaa.bbb.ccc.ddd/new</code>.</blockquote></p>
</details>
<br />
...
* Then type `git commit -m "Ready for Heroku!"` to commit all local changes in your local development environment.
...
* Then, type `git push heroku master` to push your local development code to Heroku. 
